...
const Header = ({ account, activeLocale, cart, className, links, locales, logo, search }: Props) =>
  links && (
    <header className={cn('relative bg-primary', className)}>
      <div className="header | flex h-[92px] items-center justify-between gap-1 overflow-y-visible px-4 text-white 2xl:container sm:px-10 lg:gap-8 lg:px-12 2xl:mx-auto 2xl:px-0">
            ...
            {links.map((link) =>
              link.groups && link.groups.length > 0 ? (
                <NavigationMenuPrimitive.Item key={link.href}>
                  <NavigationMenuPrimitive.Trigger className="group/button flex items-center hover:text-primary focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-primary/20">
                    <CustomLink className="p-3" href={link.href}>
                      {link.label}
                    </CustomLink>
                    <ChevronDown
                      aria-hidden="true"
                      ...
                      <ul className="flex flex-col" key={group.href}>
                        <li>
                          <NavigationMenuPrimitive.Link asChild>
                            <CustomLink className="block p-3" href={group.href}>
                              {group.label}
                            </CustomLink>

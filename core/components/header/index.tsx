...
export const Header = async ({ cart }: Props) => {
   ...
   */
  const categoryTree = data.content.pages.edges?.slice(0, 6).map(({ node }) => node) || null;

  const links = categoryTree?.map((node) => ({
    label: node.name,
    href: node.__typename !== 'ExternalLinkPage' ? node.path : node.link,
  }));
...
export const HeaderSkeleton = () => (
  <header className="t-0 relative min-h-[92px] bg-primary">
    <div className="flex animate-pulse items-center justify-between gap-1 overflow-y-visible px-4 2xl:container sm:px-10 lg:gap-8 lg:px-12 2xl:mx-auto 2xl:px-0">
      <div className="h-16 w-40 rounded" />
      ...
      <div className="flex items-center gap-2 lg:gap-4">
        ...
        <div className="h-8 w-8 rounded lg:hidden" />
      </div>
    </div>
  </header>
);
